...
class FinanceTests(APITestCase):
    ...
    def test_user_account_list(self):
        ...
        response = self.client.get("/api/v1/finance/user_account/")
        serializer_data = AccountSerializer(
            Account.objects.filter(user=self.user_1), many=True
        ).data
    ...
    def test_transfer_funds_counterparty(self):
        ...
        response = self.client.post(
            "/api/v1/finance/user_transaction/transfer_funds/", data, format="json"
        )
        senders_new_balance = Account.objects.get(number=senders_account.number).balance
        receivers_new_balance = Account.objects.get(
            number=receivers_account.number
        ).balance
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_rates(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Token " + str(self.user_1_token))
        response = self.client.get("/api/v1/finance/user_transaction/get_rates/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_user_transactions_filtred(self):
        """
        Получить историю транзакций пользователя отфильтрованных по
        типу валюты, сумме, типу транзакции
        """

        self.client.credentials(HTTP_AUTHORIZATION="Token " + str(self.user_1_token))
        response = self.client.get(
            "/api/v1/finance/user_transaction/?transaction_type=credit&currency="
            "RUR&amount=100"
        )
        serializer_data = TransactionSerializer(
            Transaction.objects.filter(
    ...
    def test_create_refill_application(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Token " + str(self.user_1_token))
        data = {"amount": "30", "type": "refill"}
        response = self.client.post(
            "/api/v1/finance/user_application/", data, format="json"
        )
